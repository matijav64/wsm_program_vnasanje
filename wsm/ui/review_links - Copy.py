    ...
    def _update_summary():
        ...
        if 'wsm_sifra' in df.columns and 'total_net' in df.columns and 'kolicina_norm' in df.columns and 'enota_norm' in df.columns:
            ...
            summary_df['avg_price_per_unit'] = summary_df.apply(
                lambda r: (r['total_net'] / Decimal(str(r['kolicina_norm']))).quantize(Decimal("0.0001")) if r['kolicina_norm'] != 0 else Decimal('0'),
                axis=1
            )
