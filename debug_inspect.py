...
def inspect_debug(file_name, header_value):
    """Preveri vsebino DEBUG CSV in primerja vsoto s podano glavo."""
    df = pd.read_csv(file_name)

    # Preverimo, ali obstaja stolpec 'vrednost'
    if "vrednost" not in df.columns:
        msg = (
            f"V datoteki {file_name} ni stolpca 'vrednost'. "
            f"Stolpci so: {df.columns.tolist()}"
        )
        print(msg)
        return
    ...
    try:
        df["vrednost_decimal"] = df["vrednost"].apply(
            lambda x: Decimal(str(x))
        )
    except Exception as e:
        print(
            "Napaka pri pretvorbi stolpca 'vrednost' v Decimal "
            f"v {file_name}: {e}"
        )
        return
    ...
    print(f"\n--- Analiza {file_name} ---")
    print(
        f"Vsota vrstic (stolpec 'vrednost'): {line_sum} €, Glava: {header} €"
    )
    print(f"Razlika: {line_sum - header} €\n")
    print("Prvih 10 vrstic (s stolpcema cena_netto, kolicina, vrednost):")
    cols_to_show = [
        col
        for col in ("cena_netto", "kolicina", "vrednost")
        if col in df.columns
    ]
    print(df[cols_to_show].head(10))

    # Poiščemo nekaj vrstic z največjimi posameznimi vsotnimi razlikami
    # glede na sorazmerni delež
    df["abs_diff_line"] = (df["vrednost_decimal"] - (header / len(df))).abs()
    top_diffs = df.sort_values("abs_diff_line", ascending=False).head(5)
    print(
        "\nNekaj vrstic z največjimi odstotnimi razlikami (glede na enak "
        "delež glave):"
    )
